...
| 1.2.9 | Añadir flag `--refresh` al CLI que ignora cache | L | Claude | haiku | 🔴 | `[ ]` | |
| 1.2.10 | Migrar `load_cache`/`save_cache` de `cli/crawl.py` a usar `CacheManager` | M | Claude | sonnet | 🔴 | `[ ]` | |
...
| # | Tarea | LOE | IA | Modelo | Prioridad | Estado | Fecha |
|---|-------|-----|-----|--------|-----------|--------|-------|
| 1.3.1 | Modificar `Track.filename_base()` en `models/track.py` | M | Claude | sonnet | 🔴 | `[ ]` | |
...
| # | Tarea | LOE | IA | Modelo | Prioridad | Estado | Fecha |
|---|-------|-----|-----|--------|-----------|--------|-------|
| 1.4.1 | Analizar `_format_sources_summary()` en `report/generator.py` | L | Gemini | flash | 🟡 | `[ ]` | |
| 1.4.2 | Identificar bug: downloaded_from se cuenta doble | L | Claude | haiku | 🟡 | `[ ]` | |
...
| # | Tarea | LOE | IA | Modelo | Prioridad | Estado | Fecha |
|---|-------|-----|-----|--------|-----------|--------|-------|
| 1.5.1 | Cambiar `requires-python` de `>=3.12` a `>=3.11` en `pyproject.toml` | L | GPT | 4o-mini | 🔴 | `[ ]` | |
| 1.5.2 | Añadir `python-dotenv` a dependencias si no está | L | GPT | 4o-mini | 🔴 | `[ ]` | |
| 1.5.3 | Añadir `load_dotenv()` al inicio de `cli/crawl.py` | L | Claude | haiku | 🔴 | `[ ]` | |
| 1.5.4 | Crear `src/core/config.py` con pydantic-settings | M | Claude | sonnet | 🟡 | `[ ]` | |
...
| # | Tarea | LOE | IA | Modelo | Prioridad | Estado | Fecha |
|---|-------|-----|-----|--------|-----------|--------|-------|
| 2.1.1 | Crear `src/providers/__init__.py` | L | GPT | 4o-mini | 🔴 | `[ ]` | |
| 2.1.2 | Crear `src/providers/base.py` con ABC `SearchProvider` | M | Claude | sonnet | 🔴 | `[ ]` | |
| 2.1.3 | Definir método abstracto `search_urls(query, num_results, site_filter) -> list[str]` | L | Claude | haiku | 🔴 | `[ ]` | |
| 2.1.4 | Añadir método `get_rate_limit() -> float` (segundos entre requests) | L | Claude | haiku | 🔴 | `[ ]` | |
...
| 2.2.5 | Parsear respuesta y extraer URLs | M | Claude | sonnet | 🔴 | `[ ]` | |
| 2.2.6 | Rate limit: 0.5 segundos (API es más tolerante) | L | Claude | haiku | 🟡 | `[ ]` | |
...
| # | Tarea | LOE | IA | Modelo | Prioridad | Estado | Fecha |
|---|-------|-----|-----|--------|-----------|--------|-------|
| 2.3.1 | Crear `src/providers/duckduckgo.py` | M | Claude | sonnet | 🟡 | `[ ]` | |
| 2.3.2 | Extraer lógica de scraping DDG de searchers existentes | M | Claude | sonnet | 🟡 | `[ ]` | |
